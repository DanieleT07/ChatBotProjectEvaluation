...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        # Get the context from the last user message in conversation history
        context = []
        if conversation_history and "Relevant Context:" in conversation_history[-2]["content"]:
            context_text = conversation_history[-2]["content"].split("Relevant Context:\n")[1]
...
async def save_state():
    try:
        save_old_embeddings(embeddings_path, vault_embeddings_tensor.tolist())
